...
<#
.SYNOPSIS
    Windows installer for PiRoute via Docker Desktop.
#>
param(
    [switch]$SkipStart
)
...
Write-Host "=== Installing PiRoute ===" -ForegroundColor Cyan
...
if (-not $SkipStart) {
    Write-Host "Building and starting PiRoute on port 5000..." -ForegroundColor Green
    Invoke-Expression "$composeCmd up --build -d"
    Write-Host "PiRoute installed. Access: http://localhost:5000" -ForegroundColor Green
} else {
